...
func (suite *OrderRepositoryTestSuite) TestGivenOrders_WhenGetOrders_ThenShouldReturnOrders() {
...
func (suite *OrderRepositoryTestSuite) TestGivenAnOrder_WhenGetOrderById_ThenShouldReturnOrder() {
	order, err := entity.NewOrder("123", 10.0, 2.0)
	suite.NoError(err)
	suite.NoError(order.CalculateFinalPrice())
	repo := NewOrderRepository(suite.Db)
	...
	orderResult, err := repo.GetOrderById(order.ID)
	suite.NoError(err)
	suite.Equal(order.ID, orderResult.ID)
	suite.Equal(order.Price, orderResult.Price)
	suite.Equal(order.Tax, orderResult.Tax)
